import { submitReviewSchema } from "@calllog/shared";
...
export async function POST(
  ...
  const scorecardVersionId = new URL(request.url).searchParams.get("scorecardVersionId");
  if (!scorecardVersionId) {
    return NextResponse.json({ error: "scorecardVersionId is required" }, { status: 400 });
  }
  ...
  if (error) {
    ...
    return NextResponse.json({ error: sanitizeError(error) }, {
