...
function tabCaptureHelpMessage() {
  return "Open the dialer/call tab, click the cauli extension icon on that tab, then press Record. Chrome only allows tab audio capture after the extension is invoked on a normal http/https page; chrome:// pages and the Extensions page cannot be recorded.";
}
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type === "MIGRATION_LIST_RECORDINGS" || msg.type === "MIGRATION_UPLOAD_RECORDINGS") {
    let senderOrigin = "";
    try {
      senderOrigin = new URL(sender.url || sender.tab?.url || "").origin;
    } catch {
      sendResponse({ success: false, error: "Invalid migration sender" });
      return;
    }
    if (senderOrigin !== globalThis.CALLLOG_COMPANION_CONFIG.webAppOrigin) {
      sendResponse({ success: false, error: "Migration sender origin is not allowed" });
      return;
    }

    if (msg.type === "MIGRATION_LIST_RECORDINGS") {
    ...
    uploadLegacyRecordings(msg.items || [])
